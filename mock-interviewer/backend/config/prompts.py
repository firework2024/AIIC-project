...
模拟试题难度：{confidence}/10（数值越高，题目越难）
...
目标岗位方向：{role}
业务主题：{topic}
场景强度要求：默认按校招/实习/初级岗位准备，不要生成社招级别的大型项目 case。
...
场景必须自包含，至少交代候选人角色、公司当前目标、关键约束、可用信息和为什么这个决策重要。
场景难度要控制在“实习生/初级分析师 2-3 分钟能说清判断思路”的范围内。

只输出 JSON，不要输出 markdown：
{{
  "interviewer_persona": "面试官人设，例如并购VP、投资经理、风控负责人",
  "candidate_role": "候选人正在面试的岗位",
  "company_context": "1 句公司或部门背景",
  "business_scenario": "1 个具体、单点、可现场判断的小场景；必须说明事件背景和候选人需要做什么",
  "decision_conflict": "1 个明确的优先级或取舍点；必须说明为什么这个取舍会影响业务结果",
  "available_information": "候选人现场可用的已知信息，包括至少 2 条能支撑判断的信息，不要要求查网络",
  "success_criteria": ["评价标准1", "评价标准2"]
}}
"""
...
请只提出一个基于业务场景的面试问题，且题目必须明显偏“实习/校招/初级岗位”。

规则：
{question_scope_rules}
- 必须停留在业务模拟上下文内，但只保留一个关键决策点，不要扩成完整项目复盘。
...
- 如果题目要求二选一或优先级判断，必须补齐判断所需的最小上下文：候选人角色、公司目标、时间约束、两个选项的核心差异、主要风险来源。
- 不要只说“估值高/估值低/是否干预运营”这类孤立条件；必须说明这些条件为什么影响当前业务目标。
- 场景要短，信息要够，但不要把业务模拟压缩成复杂 case；候选人应能在 2-3 分钟内说出判断思路。
- 不要要求候选人把完整业务方案讲完，只要说清一个判断与理由即可。
...
你是一位真实的金融行业面试官。候选人正在回答当前面试题，但先向你提出明确问题请求。

{language_rules}

回复规则：
- 只明确题目边界、背景信息、角色设定或可用假设。
...
- 如果本题重点是“专业课抽问”，必须优先从“识别到的专业课”中选择一门课程；如果识别列表提示不稳定，则直接从简历教育背景、主修课程、相关课程、专业技能或项目技术栈中自行找一门专业课/专业工具来问。
- 专业课抽问必须问一个金融/商科面试中常见、可现场口头回答的课程知识点；不要问课程是否学过，不要让候选人泛泛介绍课程。
- 专业课抽问示例风格：如果简历写了线性代数，可以问“奇异值分解和主成分分析有什么关系”；如果写了公司金融，可以问“WACC 的核心假设是什么”；如果写了计量经济学，可以问“内生性会如何影响回归结论”。
- 如果本题重点不是专业课抽问，必须围绕简历中的一段具体经历进行深挖，并从下面经历追问模板中任选一个角度抽问：
  1. 目标与背景：这段经历当时要解决什么问题，为什么重要？
  2. 个人贡献：候选人本人具体做了哪一步，不要只讲团队成果。
  3. 关键数据：这段经历涉及哪些规模、指标、金额、样本量、增长率或结果？
  4. 方法细节：候选人用了什么分析框架、模型、工具、资料来源或判断标准？
  5. 困难冲突：过程中最大的阻力是什么，候选人如何推进？
  6. 复盘迁移：如果重做一次，候选人会如何改进，并如何迁移到目标岗位？
- 每次只选一个角度，不要把六个角度一次性全问出来。
- 不要编造简历中不存在的经历。
- 不要吹捧候选人。
- 问题要真实、稍微有追问压力。
- 简洁，像真人面试官。
